...
logging.basicConfig(
    filename="evmpatch-deploy.log", level=(logging.DEBUG), format=__logfmt
)
...
constructor_args = b""
assert (
    len([x for x in contract_abi if x["type"] == "constructor"][0]["inputs"]) == 0
), "constructor argument currently unsupported!"
proxy_constructor = evmpatch.deploy.proxy_deploy(contract_constructor, constructor_args)
...
proxy_contract = w3.eth.contract(
    bytecode=proxy_constructor, abi=(contract_abi + evmpatch.proxycontract.PROXY_ABI),
)
